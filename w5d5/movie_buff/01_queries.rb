...
def harrison_ford
  ...
  Movie.select('movies.id, title').joins(:actors).where('actors.name = \'Harrison Ford\'').where('castings.ord > 1')
...
def biggest_cast
  ...
  # Movie.select('movies.id, title, COUNT(*) AS num_castings').joins(:castings).group('movies.id').order('num_castings DESC').limit(3)
  Movie.select('movies.id, title').joins(:castings).group('movies.id').order('COUNT(*) DESC').limit(3)
